...
void CUpDownClient::DrawUpStatusBar(CDC &dc, const CRect &rect, bool onlygreyrect, bool  bFlat) const
{
	...
	EMFileSize filesize = currequpfile ? (uint64)currequpfile->GetFileSize() : PARTSIZE * m_nUpPartCount;
	// wistily: UpStatusFix
...
float CUpDownClient::GetCombinedFilePrioAndCredit()
{
	...
	return (float)(10 * GetFilePrioAsNumber()) * credits->GetScoreRatio(GetIP());
}
...
uint32 CUpDownClient::GetScore(bool sysvalue, bool isdownloading, bool onlybasevalue) const
{
	...
	if (onlybasevalue)
		...
		dwBaseValue += (::GetTickCount() >= m_dwUploadTime + MIN2MS(15) ? MIN2MS(15) : MIN2MS(30));
	...
	float fBaseValue = (float)dwBaseValue / SEC2MS(1);
	if (thePrefs.UseCreditSystem())
		fBaseValue *= credits->GetScoreRatio(GetIP());

	if (!onlybasevalue)
		fBaseValue *= (float)GetFilePrioAsNumber() / 10;
