...
static COLORREF GetLogLineColor(UINT eMsgType)
{
	switch (eMsgType) {
	case LOG_SUCCESS:
...
static HBITMAP IconToBitmap(HICON hIcon, COLORREF crBackground, int cx = 16, int cy = 16)
{
	if (cx <= 0 || cy <= 0) {
		ICONINFO ii;
	...
	try {
...
#ifdef USE_METAFILE
		{
			...
			RECT rcMF = { 0, 0, (100 * (cx + 1) * iWidthMM) / iWidthPels, (100 * (cy + 1) * iHeightMM) / iHeightPels };
